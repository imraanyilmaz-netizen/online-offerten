...
const umzugServiceOptions: ServiceOption[] = [
  ...
  { id: 'privatumzug_reinigung', label: 'Privatumzug und Reinigung', url: '/kostenlose-offerte-anfordern?service=umzug&step=3&umzugArt=privatumzug&endreinigung=ja' },
    ...
    >
      <div className="flex flex-col md:flex-row gap-4 rounded-xl md:p-4 lg:p-0">
        <div className="flex-1 min-w-0">
          <label className="block text-sm font-medium text-foreground mb-2 text-left">Was steht an?</label>
          <div className="relative">
            <input
              ...
              className="w-full px-5 py-4 pr-12 md:pr-40 border-2 border-border rounded-3xl focus:border-green-500 dark:focus:border-emerald-500 focus:outline-none text-foreground bg-background"
            ...
            {serviceInput && (
              <button
                ...
                className="absolute right-32 md:right-32 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground hidden md:block"
              >
                <X className="h-5 w-5" />
              </button>
            )}
            ...
            {showDropdown && filteredOptionsMemo.length > 0 && (
              <div
                ref={dropdownRef}
                className="absolute z-50 w-full mt-2 bg-popover border-2 border-border rounded-lg max-h-64 overflow-y-auto shadow-md"
              >
                <div className="py-1">
                  <div className="px-4 py-2.5 text-xs font-bold text-green-700 dark:text-emerald-300 uppercase tracking-wider bg-green-50 dark:bg-emerald-950/50 border-b border-green-100 dark:border-emerald-900">
                    Umzug
                  </div>
                  {filteredOptionsMemo.map((option) => (
                    <button
                      ...
                      className="w-full text-left px-4 py-3 hover:bg-green-50 dark:hover:bg-emerald-950/40 transition-colors flex items-center justify-between group border-b border-border/60 last:border-b-0"
                    >
                      <span className="text-foreground font-medium group-hover:text-green-700 dark:group-hover:text-emerald-400">{option.label}</span>
                      <ChevronDown className="h-4 w-4 text-muted-foreground group-hover:text-green-600 dark:group-hover:text-emerald-400 rotate-[-90deg] transition-colors" />
                    </button>
                  ))}
                </div>
              </div>
      ...
      <div className="mt-4 pt-4 border-t border-border">
        <p className="flex flex-wrap items-center gap-1.5 text-sm md:text-base text-muted-foreground leading-relaxed">
          <Clock className="h-4 w-4 text-muted-foreground flex-shrink-0" />
          <span>Die letzte Anfrage wurde vor</span>
          <span className="font-semibold text-foreground">{minutesAgo} Minuten</span>
          <span>gestellt</span>
        </p>
      </div>
    </form>
