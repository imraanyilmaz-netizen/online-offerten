...
const ChecklistItem = ({ id, text, isChecked, onToggle }) => {
  return (
    <motion.div
    ...
    >
      <Checkbox id={id} checked={isChecked} onCheckedChange={onToggle} className="w-5 h-5" />
...
const ChecklistSection = ({ section, checkedItems, onToggleItem }) => {
  const totalItems = Array.isArray(section.items) ? section.items.length : 0;
  const completedItems = Array.isArray(section.items) ? section.items.filter((item, index) => checkedItems[`${section.id}-${index}`]).length : 0;
...
const ChecklistsPage = () => {
  ...
  const canonicalUrl = "https://online-offerten.ch/checklisten";
