...
const PurchasedByCard = ({ partner, isReviewable, onReviewClick }) => {
    ...
    >
      <div className="p-4 flex-grow">
        <div className="flex items-start gap-4">
          <Link href={`/partner/${partner.slug}`} className="flex-shrink-0">
            <img
          ...
          <div className="flex-1">
            <div className="flex justify-between items-start">
              <Link href={`/partner/${partner.slug}`} className="hover:underline">
...
const AddressCard = ({ title, icon, street, zip, city, floor, lift, rooms }) => {
    if (!zip && !city && !street) {
        return null;
    }
    return (
...
const QuoteStatusPage = () => {
  ...
  const [purchasingPartners, setPurchasingPartners] = useState([]);
      ...
      try {
        ...
        if (quoteData.status === 'sold_out' || quoteData.status === 'quota_filled' || quoteData.status === 'approved') {
          ...
          if (purchasedData && purchasedData.length > 0) {
            const partnerIds = purchasedData.map(p => p.partner_id);
            ...
            <Card className="shadow-lg overflow-hidden">
              <CardHeader>
                ...
                <CardDescription className="text-center text-gray-600">
                  {quote ? `Anfrage-ID: ${quoteId.substring(0, 8)}` : 'Ihre Anfrage wird geladen...'}
                </CardDescription>
              </CardHeader>
              <CardContent>
