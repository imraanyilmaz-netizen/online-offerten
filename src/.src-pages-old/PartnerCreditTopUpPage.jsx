...
const PartnerCreditTopUpPage = () => {
  // Removed useTranslation
  const { user } = useAuth();
  const { toast } = useToast();
  const [amount, setAmount] = useState('100');
  const [minAmount, setMinAmount] = useState(null); // Will be loaded from admin settings
  ...
  const handleTopUp = useCallback(async () => {
    ...
    try {
      ...
      toast({
        ...
        description: e.message || 'Die Zahlung konnte nicht verarbeitet werden. Bitte versuchen Sie es erneut.',
      });
    } finally {
      setIsLoading(false);
    }
...
export default PartnerCreditTopUpPage;