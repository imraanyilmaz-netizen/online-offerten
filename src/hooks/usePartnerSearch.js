  ...
  const fetchPartners = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('partners')
        .select('id, company_name, slug, address_street, address_city, address_zip, main_categories, offered_services, service_regions, average_rating, review_count, badge_tier, logo_url, hero_image_url')
        .eq('status', 'active');
      ...
      filtered = filtered.filter(partner => {
        ...
        if (partner.offered_services && partner.offered_services.length > 0) {
          const categoryServiceMap = {
            'umzug': ['privatumzug', 'geschaeftsumzug', 'internationaler_umzug', 'spezialtransport', 'kleintransport', 'moebellift_service', 'raeumung_service', 'entsorgung_service'],
